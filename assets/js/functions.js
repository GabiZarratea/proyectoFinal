...
const crearPlantilla = (producto) => {
  ...
  const articulosSeleccionados = JSON.parse(localStorage.getItem("seleccionados")).find(prod => prod._id == producto._id)
  ...
  return `<div class="card" style="width: 20rem; text-align: center;" >
            ...
            <div class="card-body d-flex justify-content-around">
              ...
              <a href="${href}?id=${producto._id}" class="btn btn-details"> Detalles </a>
            </div>
            <div>
              ${producto.disponibles < 5 ? "<p class='bg-danger text-white fs-6'><img src='../images/advertencia.svg'></img> Pocas unidades disponibles </p>" : "<br><br>"} 
            </div>
          </div>`;
...
const crearPlantillaCarrito = (producto) => {
  ...
  const articulosSeleccionados = JSON.parse(localStorage.getItem("seleccionados"))
    ...
    return `<div class="card" style="width: 20rem; text-align: center;" >
                        ...
                        <button data-id="${producto._id}" class="ms-5 btn btn-carrito-vacio btn-success ${articulosSeleccionados ? 'btn-danger' : 'btn-success'}"></button>
                        </div> 
                      </div>`;
  };
